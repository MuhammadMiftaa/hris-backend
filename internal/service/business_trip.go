...
func (s *businessTripService) Create(ctx context.Context, employeeID uint, roleLevel string, req dto.CreateBusinessTripRequest) (dto.BusinessTripRequestResponse, error) {
	...
	m := model.BusinessTripRequest{
		...
		Status:      model.RequestStatusEnum(status),
	...
	if isAdminSubmission {
		for d := sDate; !d.After(eDate); d = d.AddDate(0, 0, 1) {
			...
			log, _ := s.attendRepo.GetTodayLog(ctx, tx, created.EmployeeID, dateStr)
			if log != nil {
				upd := map[string]interface{}{
					"status":                   string(model.AttendanceBusinessTrip),
					"business_trip_request_id": created.ID,
				...
				newLog := model.AttendanceLog{
					...
					BusinessTripRequestID: &created.ID,
